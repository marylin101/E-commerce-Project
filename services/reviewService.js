...
const createReview = async (data) => {
    ...
    if(!data.productId || !data.userId || data.rating===undefined || data.rating=== null){
        throw ApiError.badRequest('There are missing required fields: productId, userId, rating are required.');
    }
    if(Number.isNaN(ratingNum) ||ratingNum < 1 || ratingNum > 5){
        throw ApiError.badRequest('Rating must be a value between 1 and 5.');
    }
    const product = await productRepository.findProductById(data.productId);
    if(!product){
        throw ApiError.notFound("Review for this product with ID " + data.productId + " was not found.");
    }
...
const getReviewsByProductId = async (productId) => {
    ...
    if(!product){
        throw ApiError.notFound("The product with the provided ID:" +productId+ " was not found.");
    }
